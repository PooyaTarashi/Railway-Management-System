...
class Railway_System:
    ...
    def decision_maker(self, reserve_time:datetime, origin, destination, seat_type) -> Vehicle:
    ...
    def book(self, cmd):
        ...
        if vehicle == "Auto":
            vehicle = self.decision_maker(reserve_time, origin, destination, seat_type)
    ...
    def cut_the_capacity(self, cmd):
        ...
        available_percentage = int(cmd[6]) / 100
        # cut the capacity of vehicles
        # vehicle:Vehicle = None    # just wrote these two lines to get suggestions from vscode, not necessarily needed.
        # reserve:Reservation = None
        for vehicle in self._vehicles.values():
            if vehicle.vehicle_type == vehicle_type:
                # print(vehicle.primary_capacity)
                # tmp_dict = OrderedDict()
                for seat in vehicle.primary_capacity.keys():
                    vehicle.primary_capacity[seat] *= ((1 - available_percentage) * vehicle.primary_capacity[seat])
                    vehicle.primary_capacity[seat] = int(vehicle.primary_capacity[seat])

        for vehicle in self._vehicles.values():
            if vehicle.vehicle_type == vehicle_type:
                reserved_seats_counter = OrderedDict()
                new_capacity_dict = vehicle.primary_capacity
                sacking_passengers_count = OrderedDict()      # reserved_seats_dict - new_capacity
                # define reserved_seats_counter with value of zero
                for key in new_capacity_dict.keys():
                    reserved_seats_counter[key] = 0
                for reserve in self._reservations:
                    if reserve.vehicle == vehicle:
                        reserved_seats_counter[reserve.seat_type] += 1
                # now, the dictionary is defined, let's subtract each value in prime_capacity_dict from reserved_seats_dict
                for key in new_capacity_dict:
                    sacking_passengers_count[key] = reserved_seats_counter[key] - new_capacity_dict[key]
                    if sacking_passengers_count[key] < 0:
                        sacking_passengers_count[key] = 0
                # the subtracts are ready to use let's use another loop for reserve
                for reserve in reversed(self._reservations):
                    if reserve.vehicle == vehicle and sacking_passengers_count[reserve.seat_type] > 0:
                        reserve.vehicle.capacity[reserve.seat_type] = 0     # assigns zero to capacity type which is already full
                        sacking_passengers_count[reserve.seat_type] -= 1
        ...
        # ok let's change capacity
        for vehicle in self._vehicles.values():
            if vehicle.vehicle_type == vehicle_type:
                for key in vehicle.capacity.keys():
                    vehicle.capacity[key] = vehicle.primary_capacity[key] - (vehicle.const_primary_capacity[key] - vehicle.capacity[key])
